...
\begin{document}
  ...
  \item \textbf{XOR} \textit{Include a picture of the network
    ...
    What is probably happening is that the weights of the neural net
    are either stuck in a local minimum or is not convergent.
    These are fixed respectively by having random restarts and considering
    the best performing neural network, and by reducing learning rate
    to ensure convergence. 

\edit{Which of these is the case? what about getting stuck - what
are the weights initialized to?}
...
%    We could reduce learning rate to make this less likely. 
  ...
  \item \textbf{Experimental Analysis}
    ...
    \item \textit{For this learning rate, chart the training set and
      ...
      \item \textit{Why is it important that we use a validation set
        ...
        If we use the test set to tune the number of epochs, 
        upon testing we will achieve artificially better reported 
        performance. 
  ...
  \item \textbf{Model Selection}
    ...
    \item \textit{Experiment with the distributed and binary encoding
      ...
      digits that links all odd digits together, even though they all share
      a 1 in the least significant spot in a binary encoding, for 
      example. In other words, there is no intrinsic numerical ordering
      of images of numbers - i.e., the desired output is categorical, 
      even though there is an ordering in the numbers themselves. 
      (Additionally, it would not appear that a binary encoding would
      even be optimal for ordered outputs by the previous argument.)
