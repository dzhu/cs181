...
\end{verbatim}
  ...
  \item \emph{Alpha and beta values get small for long sequences. If
    ...
    product of the normalization factors at each step.

    Note that, since the calculations are linear in the alphas,
    normalizing at each step, then multiplying the factors, gives the
    same result as not normalizing until the end, then taking the
    resulting normalization factor. But the result of that would be
    \begin{align*}
