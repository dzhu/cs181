...
\item
  ...
  \item \emph{Plot $\log(P(D|\Theta))$ for training the model for Boston with
    ...
    hidden state, the HMM classifies the test data perfectly. But we must keep
    in mind that likelihoods tend to get very small anyway. This example shows how
  ...
  \item \emph{By looking at the data and the learned models, try to explain your
    ...
    However, as a tradeoff, more hidden states means much more time to process, 
